public class Perlin
{
    private static readonly int[] s_permutation = { 151,160,137,91,90,15,					// Hash lookup table as defined by Ken Perlin.  This is a randomly
    ...
    private static readonly int[] s_p;                                                    // Doubled permutation to avoid overflow
    ...
    public double OctavePerlin(double x, double y, double z, int octaves, double persistence)
    {
        ...
        {
            total += GetPerlinValue(x * frequency, y * frequency, z * frequency) * amplitude;
    ...
    public double GetPerlinValue(double x, double y, double z)
    {
        if (Repeat > 0)
        ...
        var aaa = s_p[s_p[s_p[xi] + yi] + zi];
        var aba = s_p[s_p[s_p[xi] + Inc(yi)] + zi];
        var aab = s_p[s_p[s_p[xi] + yi] + Inc(zi)];
        var abb = s_p[s_p[s_p[xi] + Inc(yi)] + Inc(zi)];
        var baa = s_p[s_p[s_p[Inc(xi)] + yi] + zi];
        var bba = s_p[s_p[s_p[Inc(xi)] + Inc(yi)] + zi];
        var bab = s_p[s_p[s_p[Inc(xi)] + yi] + Inc(zi)];
        var bbb = s_p[s_p[s_p[Inc(xi)] + Inc(yi)] + Inc(zi)];
